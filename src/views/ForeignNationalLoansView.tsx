...
export async function getForeignNationalLoansMetadata(
  locale: "en" | "es",
): Promise<Metadata> {
  const isEs = locale === "es";
  const t = await getTranslations({
    locale,
    namespace: "ForeignNationalLoans.meta",
  });

  const canonicalUrl = isEs
    ? "https://www.myloandesk.com/es/loan-programs/foreign-national-loans/"
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: "https://www.myloandesk.com/loan-programs/foreign-national-loans/",
        es: "https://www.myloandesk.com/es/loan-programs/foreign-national-loans/",
        "x-default":
          "https://www.myloandesk.com/loan-programs/foreign-national-loans/",
      },
    },
...
export default async function ForeignNationalLoansView({
  locale,
}: {
  locale: "en" | "es";
}) {
  ...
  const t = await getTranslations({
    locale,
    namespace: "ForeignNationalLoans",
  });
  const isEs = locale === "es";

  const homeHref = isEs ? "/es" : "/";
  const loanProgramsHref = isEs ? "/es/loan-programs" : "/loan-programs";
  const foreignNationalHref = isEs
    ? "/es/loan-programs/foreign-national-loans"
  ...
  const breadcrumbItems = [
    ...
    { name: t("breadcrumbs.foreign_national"), url: foreignNationalHref },
  ];
  ...
  const overviewBeneficiaries = t.raw("overview.side_card.items") as string[];
  const documentationItems = t.raw("documentation.items") as string[];
  const faqItems = t.raw("faq.items") as { q: string; a: string }[];

  const propertyOptionsCards = [
    {
      title: t("property_options.cards.0.title"),
      desc: t("property_options.cards.0.desc"),
      icon: Building2,
    },
    {
      title: t("property_options.cards.1.title"),
      desc: t("property_options.cards.1.desc"),
      icon: Home,
    },
    {
      title: t("property_options.cards.2.title"),
      desc: t("property_options.cards.2.desc"),
      icon: Building,
    },
    {
      title: t("property_options.cards.3.title"),
      desc: t("property_options.cards.3.desc"),
      icon: FileCheck,
    },
  ];
  ...
  return (
    <main className="min-h-screen bg-primary-bg pt-6 pb-12 sm:pt-8 sm:pb-16 lg:pt-10 lg:pb-24 text-ink">
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-5 bg-cream/40 border border-line p-6 sm:p-8 rounded-3xl">
            ...
            <ul className="space-y-3 text-xs sm:text-sm text-ink">
              {overviewBeneficiaries.map((item, idx) => (
      ...
      <section className="py-16 sm:py-24 border-t border-b border-line bg-cream/20 mb-16 sm:mb-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            >
              {t("property_options.heading")}
            </h2>
            <p className="text-xs sm:text-sm text-ink-2 mt-2">
              {t("property_options.subtitle")}
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {propertyOptionsCards.map((item, idx) => (
              <div
                key={idx}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 mb-16 sm:mb-24">
        <div className="bg-cream/30 border border-line rounded-3xl p-8 sm:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="max-w-xl">
            ...
            >
              {t("credit_flexibility.heading")}
            </h2>
            <p className="text-xs sm:text-sm text-ink-2 leading-relaxed mb-4">
              {t("credit_flexibility.p1")}
            </p>
            <p className="text-xs sm:text-sm text-ink-2 leading-relaxed">
              {t("credit_flexibility.p2")}
            </p>
          </div>
          ...
          <div className="w-full md:w-auto shrink-0 text-center">
            ...
            >
              {t("credit_flexibility.cta_button")}
              <ArrowRight size={18} />
            </Link>
          </div>
        </div>
      ...
      <ForeignNationalCalculator
        pagePath={foreignNationalHref}
        locale={locale}
      />
