...
export async function getCommercialRealEstateLoansMetadata(
  locale: "en" | "es",
): Promise<Metadata> {
  const isEs = locale === "es";
  const t = await getTranslations({
    locale,
    namespace: "CommercialRealEstateLoans.meta",
  });

  const canonicalUrl = isEs
    ? "https://www.myloandesk.com/es/loan-programs/commercial-real-estate-loans/"
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: "https://www.myloandesk.com/loan-programs/commercial-real-estate-loans/",
        es: "https://www.myloandesk.com/es/loan-programs/commercial-real-estate-loans/",
        "x-default":
          "https://www.myloandesk.com/loan-programs/commercial-real-estate-loans/",
      },
    },
...
export default async function CommercialRealEstateLoansView({
  locale,
}: {
  locale: "en" | "es";
}) {
  ...
  const t = await getTranslations({
    locale,
    namespace: "CommercialRealEstateLoans",
  });
  const isEs = locale === "es";
  ...
  const breadcrumbItems = [
    ...
    { name: t("breadcrumbs.commercial_real_estate"), url: creHref },
  ];
  ...
  const waysToFinanceCards = t.raw("ways_to_finance.cards") as {
    badge: string;
    title: string;
    desc: string;
  }[];
  ...
  const specialUseTags = t.raw("special_use.tags") as string[];
          ...
          <table className="my-[18px] mb-[30px] w-full border-collapse text-[13.5px]">
            <thead>
              <tr className="bg-[#1C1C1C] text-left font-bold text-white">
                ...
                <th className="p-[11px_14px]">{t("comparison.th_sba7a")}</th>
                <th className="p-[11px_14px]">{t("comparison.th_sba504")}</th>
            ...
            <tbody>
              <tr className="border-b border-[#E6E0D3]">
                ...
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.down_payment.sba7a")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.down_payment.sba504")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
              ...
              <tr className="border-b border-[#E6E0D3] bg-[#F3EFE6]">
                ...
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.loan_amount.sba7a")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.loan_amount.sba504")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.loan_amount.conv")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                  {t("comparison.rows.working_capital.label")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.working_capital.sba7a")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.working_capital.sba504")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.working_capital.conv")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3] bg-[#F3EFE6]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                  {t("comparison.rows.rate_structure.label")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.rate_structure.sba7a")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.rate_structure.sba504")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.rate_structure.conv")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                ...
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.best_for.sba7a")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.best_for.sba504")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
