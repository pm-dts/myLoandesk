...
export async function getVALoansMetadata(
  locale: "en" | "es",
): Promise<Metadata> {
  const isEs = locale === "es";
  const t = await getTranslations({
    locale,
    namespace: "VALoans.meta",
  });

  const canonicalUrl = isEs
    ? "https://www.myloandesk.com/es/loan-programs/va-loans/"
    : "https://www.myloandesk.com/loan-programs/va-loans/";
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: "https://www.myloandesk.com/loan-programs/va-loans/",
        es: "https://www.myloandesk.com/es/loan-programs/va-loans/",
        "x-default": "https://www.myloandesk.com/loan-programs/va-loans/",
      },
    },
    openGraph: {
      title: t("og_title"),
...
export default async function VALoansView({ locale }: { locale: "en" | "es" }) {
  ...
  const vaLoansHref = isEs
    ? "/es/loan-programs/va-loans"
  ...
  const breadcrumbItems = [
    ...
    { name: t("breadcrumbs.va_loans"), url: vaLoansHref },
  ];
  ...
  const fundingFeeRows = t.raw("funding_fee_rates.rows") as {
    down: string;
    first: string;
    subsequent: string;
  }[];
  ...
  return (
    <div className="min-h-screen bg-[#EDEAE2] text-[#55524C] antialiased">
      ...
      <section className="mx-auto max-w-[900px] bg-[#FBF8F2] px-[22px] py-8 sm:px-14 sm:py-[44px] sm:pb-[60px]">
        ...
        {/* Funding Fee Rates */}
        <h2 className="mb-3.5 mt-[42px] font-serif text-2xl font-bold leading-[1.25] text-[#1C1C1C]">
          {t("funding_fee_rates.title")}
        </h2>
        <p className="mb-3.5 max-w-[700px] text-[15px] leading-[1.75] text-[#55524C]">
          {t("funding_fee_rates.p1")}
        </p>
            ...
            <thead>
              <tr className="border-b-2 border-[#E6E0D3] bg-[#F3EFE6] text-left font-bold text-[#1C1C1C]">
                <th className="p-[9px_12px]">
                  {t("funding_fee_rates.th_down")}
                </th>
                <th className="p-[9px_12px]">
                  {t("funding_fee_rates.th_first")}
                </th>
                <th className="p-[9px_12px]">
                  {t("funding_fee_rates.th_subsequent")}
                </th>
              </tr>
            </thead>
            <tbody>
              {fundingFeeRows.map((row, idx) => (
                <tr key={idx} className="border-b border-[#E6E0D3]">
                  <td className="p-[9px_12px] text-[#55524C]">{row.down}</td>
                  <td className="p-[9px_12px] text-[#55524C]">{row.first}</td>
          ...
          <table className="my-[18px] mb-[30px] w-full border-collapse text-[13.5px]">
            <thead>
              <tr className="bg-[#1C1C1C] text-left font-bold text-white">
                ...
                <th className="p-[11px_14px]">{t("comparison.th_va")}</th>
                <th className="p-[11px_14px]">{t("comparison.th_fha")}</th>
            ...
            <tbody>
              ...
              <tr className="border-b border-[#E6E0D3]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                  {t("comparison.rows.fee.label")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.fee.va")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.fee.fha")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.fee.conv")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3] bg-[#F3EFE6]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                ...
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.max_loan.va")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.max_loan.fha")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
