...
export async function getJumboLoansMetadata(
  locale: "en" | "es",
): Promise<Metadata> {
  const isEs = locale === "es";
  const t = await getTranslations({
    locale,
    namespace: "JumboLoans.meta",
  });

  const canonicalUrl = isEs
    ? "https://www.myloandesk.com/es/loan-programs/jumbo-loans/"
    : "https://www.myloandesk.com/loan-programs/jumbo-loans/";
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: "https://www.myloandesk.com/loan-programs/jumbo-loans/",
        es: "https://www.myloandesk.com/es/loan-programs/jumbo-loans/",
        "x-default": "https://www.myloandesk.com/loan-programs/jumbo-loans/",
      },
    },
    openGraph: {
      title: t("og_title"),
...
export default async function JumboLoansView({
  locale,
}: {
  locale: "en" | "es";
}) {
  ...
  const t = await getTranslations({
    locale,
    namespace: "JumboLoans",
  });
  const isEs = locale === "es";

  const homeHref = isEs ? "/es" : "/";
  const loanProgramsHref = isEs ? "/es/loan-programs" : "/loan-programs";
  const jumboHref = isEs
    ? "/es/loan-programs/jumbo-loans"
  ...
  const breadcrumbItems = [
    ...
    { name: t("breadcrumbs.jumbo_loans"), url: jumboHref },
  ];
  ...
  const reserveRows = t.raw("down_payment_reserves.rows") as {
    amount: string;
    guidance: string;
  }[];
    ...
    <div className="min-h-screen bg-[#EDEAE2] text-[#55524C] antialiased">
      ...
      <section className="mx-auto max-w-[900px] bg-[#FBF8F2] px-[22px] py-8 sm:px-14 sm:py-[44px] sm:pb-[60px]">
        ...
        <p className="mb-3.5 max-w-[700px] text-[15px] leading-[1.75] text-[#55524C]">
          {t("what_is.p1_part1")}
          <strong>{t("what_is.p1_baseline")}</strong>
          {t("what_is.p1_part2")}
          <strong>{t("what_is.p1_highcost")}</strong>
          {t("what_is.p1_part3")}
        </p>
        <p className="mb-3.5 max-w-[700px] text-[15px] leading-[1.75] text-[#55524C]">
          {t("what_is.p2")}
        ...
        <JumboPathSelector locale={locale} />
        ...
        <h2 className="mb-3.5 mt-[42px] font-serif text-2xl font-bold leading-[1.25] text-[#1C1C1C]">
          {t("down_payment_reserves.title")}
        </h2>
        <p className="mb-3.5 max-w-[700px] text-[15px] leading-[1.75] text-[#55524C]">
          {t("down_payment_reserves.p1")}
        </p>
        <table className="my-3.5 mb-2 max-w-[600px] w-full border-collapse text-[13.5px]">
          <thead>
            <tr className="border-b-2 border-[#E6E0D3] bg-[#F3EFE6] text-left font-bold text-[#1C1C1C]">
              <th className="p-[9px_12px]">
                {t("down_payment_reserves.th_loan_amount")}
              </th>
              <th className="p-[9px_12px]">
                {t("down_payment_reserves.th_guidance")}
              </th>
            </tr>
          </thead>
          <tbody>
            {reserveRows.map((row, idx) => (
              <tr key={idx} className="border-b border-[#E6E0D3]">
                <td className="p-[9px_12px] text-[#55524C]">{row.amount}</td>
                <td className="p-[9px_12px] text-[#55524C]">{row.guidance}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="text-[13px] leading-[1.75] text-[#8F8A7C]">
          {t("down_payment_reserves.footnote")}
        </p>
            ...
            <thead>
              <tr className="bg-[#1C1C1C] text-left font-bold text-white">
                ...
                <th className="p-[11px_14px]">
                  {t("comparison.th_conforming")}
                </th>
                <th className="p-[11px_14px]">{t("comparison.th_jumbo")}</th>
                <th className="p-[11px_14px]">
                  {t("comparison.th_super_jumbo")}
                </th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-[#E6E0D3]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                  {t("comparison.rows.loan_amount.label")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.loan_amount.conforming")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.loan_amount.jumbo")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.loan_amount.super_jumbo")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3] bg-[#F3EFE6]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                  {t("comparison.rows.pmi.label")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.pmi.conforming")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.pmi.jumbo")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.pmi.super_jumbo")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                  {t("comparison.rows.paths.label")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.paths.conforming")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.paths.jumbo")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.paths.super_jumbo")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3] bg-[#F3EFE6]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                  {t("comparison.rows.sold_to_agency.label")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.sold_to_agency.conforming")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.sold_to_agency.jumbo")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.sold_to_agency.super_jumbo")}
                </td>
              </tr>
              <tr className="border-b border-[#E6E0D3]">
                <td className="p-[11px_14px] font-bold text-[#1C1C1C]">
                ...
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.best_for.conforming")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.best_for.jumbo")}
                </td>
                <td className="p-[11px_14px] text-[#55524C]">
                  {t("comparison.rows.best_for.super_jumbo")}
                </td>
              </tr>
            </tbody>
          </table>
